...
function [feats,win_posw,win_posh,winw,winh] = ...
    compute_features_fast(im,nori,full_360,border,window_size,block_sizes,nlevels,...
...
%      I = padarray(I,[padh padw],'replicate');
      ...
      [loch,locw] = meshgrid(offseth:strideh:h-IH-border+1,...
                             offsetw:stridew:w-IW-border+1);
      tdesc =tdesc+length(loch(:));
    end
  
    td = 0; 
    for ii=1:length(gw), 
        td=td+(size(gw{ii},1)-1)*(size(gw{ii},2)-1); 
    end
    fdim = td*nori;
...
%    level_feats_lin = compute_gradient_features_lin(R,IW,IH,locw(:),loch(:),gw,gh);

%    level_feats_nn = compute_gradient_features_nn(R,IW,IH,locw,loch,gw,gh);
%    new_level_feats_nn = compute_gradient_features_nn(newR,IW,IH,locw,loch,gw,gh);
    ...
    f = mex_feature(single(newR),int32([loch(:),locw(:)]),...
                        int32(my_bounds));
